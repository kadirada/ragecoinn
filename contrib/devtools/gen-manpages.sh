...
LITECOIND=${LITECOIND:-$SRCDIR/ragecoind}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/ragecoin-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/ragecoin-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/ragecoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
RAGEVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
